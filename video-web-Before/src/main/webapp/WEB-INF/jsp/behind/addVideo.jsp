...
<head>
    ...
    <script type="text/javascript">
        ...
        //页面加载完毕之后就执行以下代码片段
        $(function () {
            var speakerId = '${video.speaker.id}';
        ...
        function uploadImg(){
            ...
            $.ajaxFileUpload({
                ...
                fileElementId : "image", //上传文件选择框的id属性
...
<div class="container" style="margin-top: 20px;">
    ...
    <form class="form-horizontal" action="${pageContext.request.contextPath}/video/saveOrUpdate" method="post">
        ...
        <c:if test="${empty video.playNum != null}">
            <input type="hidden" name="playNum" value="${video.playNum}">
        </c:if>
        <input type="hidden" name="playNum" value=0>

        <div class="form-group">
            <label for="inputEmail3" class="col-sm-2 control-label">封面图片地址</label>
            <div class="col-sm-10">
                <input type="file" name="image" onchange="uploadImg()" id="image">
                <c:if test="${not empty video.imageUrl}">
                    <div id="photo"><img  width='100px' height='100px' src='${video.imageUrl}'/></div>
                </c:if>
                <c:if test="${empty video.imageUrl}">
                    <div id="photo"></div>
                </c:if>
            </div>
            <input type="hidden" name="imgUrl" id="imgUrl" value=" ${video.imageUrl}">
            <input type="hidden" name="playNum" value="${video.playNum}">
        </div>
        <div class="form-group">
